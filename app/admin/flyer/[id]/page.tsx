...
export default async function FlyerPage({
  ...
  const coverUrl = cover?.url ?? "https://placehold.co/1200x675?text=Sem+foto"

  const headerList = await headers()
  const rootDomain = process.env.NEXT_PUBLIC_ROOT_DOMAIN ?? ""
  const proto = headerList.get("x-forwarded-proto") ?? "https"

  // Monta a URL pública correta do veículo
  const storeHost =
    vehicle.store.customDomain ??
    (vehicle.store.subdomain ? `${vehicle.store.subdomain}.${rootDomain}` : rootDomain)

  const publicUrl = `${proto}://${storeHost}/veiculo/${vehicle.id}`
            ...
            <div className="grid gap-3 sm:grid-cols-3 lg:grid-cols-4">
              <div className="rounded-lg border border-white/15 bg-white/5 p-5">
                <div className="flex items-center gap-2">
                  ...
                  <p className="text-[11px] font-medium text-neutral-300">Ano</p>
                </div>
                <p className="mt-2 text-xl font-semibold text-white">{vehicle.year}</p>
              </div>
              ...
              <div className="rounded-lg border border-white/15 bg-white/5 p-5">
                <div className="flex items-center gap-2">
                  ...
                  <p className="text-[11px] font-medium text-neutral-300">Quilometragem</p>
                </div>
              ...
              <div className="rounded-lg border border-white/15 bg-white/5 p-5">
                <div className="flex items-center gap-2">
                  ...
                  <p className="text-[11px] font-medium text-neutral-300">Combustível</p>
                </div>
          ...
          <div className="flex flex-col justify-between rounded-lg border border-white/15 bg-white/5 p-5">
            <div className="flex items-center justify-between gap-4">
              <div className="min-w-0 space-y-1">
                <p className="text-xs font-medium text-neutral-300">Ver anúncio completo</p>
                <p className="break-all text-xs font-medium text-neutral-100">{publicUrl}</p>
              </div>
            ...
            <p className="mt-3 text-center text-[11px] text-neutral-300">
              Aponte a câmera do celular para acessar a ficha completa,
              <br />
